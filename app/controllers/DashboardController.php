...
final class DashboardController
{
    public static function summary(): void
    {
        ...
        try {
            $data = Cache::remember("dashboard_summary_{$accountId}", function () {
            ...
            Response::ok([
                'totals' => ['events' => 0, 'sessions' => 0, 'students' => 0, 'courses' => 0, 'exams' => 0, 'active_exams' => 0, 'suspicious_sessions' => 0, 'suspicious_students' => 0, 'events_last_hour' => 0],
                'system' => ['last_event_at' => null, 'last_aggregation_at' => null, 'pending_events' => 0],
            ]);
        }
    }
    ...
    public static function eventsOverTime(): void
    {
        ...
        try {
            ...
            error_log("[DashboardController::eventsOverTime] Error: " . $e->getMessage());
            Response::ok(['range' => $range, 'points' => []]);
        }
    }
    ...
    public static function eventTypes(): void
    {
        ...
        try {
            ...
            error_log("[DashboardController::eventTypes] Error: " . $e->getMessage());
            Response::ok(['types' => []]);
        }
    }
    ...
    public static function eduOverview(): void
    {
        ...
        try {
            $data = Cache::remember("edu_overview_{$accountId}_" . ($isOwner ? 'owner' : 'tenant'), function () use ($accountId, $isOwner) {
                ...
                $moodleCourseIds = array_filter(array_map('intval', array_column($courses, 'moodle_course_id')), fn($id) => $id > 0);
                $examsByCourse = [];
                ...
                $topSuspicious = Database::fetchAll(
                    'SELECT ss.student_id, ss.risk_score, ss.risk_level,
                            ...
                            COALESCE(e.name, CONCAT("امتحان #", ss.exam_id)) AS exam_name,
                            COALESCE(e.id, ss.exam_id) AS exam_id,
            ...
            error_log("[DashboardController::eduOverview] Error: " . $e->getMessage());
            Response::ok([
                'courses' => [],
                'risk_distribution' => [],
                'threats' => [],
